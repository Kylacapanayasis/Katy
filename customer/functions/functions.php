...
function getCustomerDetails() {
    ...
    if (isset($_SESSION['customer_email'])) {
        ...
        $run_customer_id = mysqli_query($db, $get_customer_id);

        if ($run_customer_id && $row = mysqli_fetch_assoc($run_customer_id)) {
            return $row['customer_id'];
        } else {
            // Add a debugging statement
            echo "Failed to retrieve customer ID from the database!";
            return null;
        }
    } else {
        // Add a debugging statement
...
function add_cart(){
    ...
    if(isset($_GET['add_cart'])){
        ...
        $sub_total = $product_qty*$pro_price;
        ...
        if(mysqli_num_rows($run_check)>0){
            ...
            $query = "insert into cart (p_id,customer_id,qty,total) values ('$p_id','$c_id','$product_qty','$total')";
...
function getPro(){
    ...
    $get_products = "select * from products order by 1 DESC LIMIT 8,4";
...
function getpcatpro(){
    ...
    if(isset($_GET['p_cat'])){
        ...
        $get_p_cat ="select * from product_classifications where p_cat_id='$p_cat_id'";
...
function total_price(){
    ...
    while($record=mysqli_fetch_array($run_cart)){
        ...
        $pro_roast = $record['roast'];

        $pro_coffee = $record['coffee'];
